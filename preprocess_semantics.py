...
Runs Grounding DINO + SAM2 on every frame in the dataset to produce:
  ...
  2. A per-pixel drivability score   (float16, H x W) -> semantics/<frame>_drv.npy
...
def segment_frame(img_path, sam2_predictor, grounding_model, device,
    ...
    return masks.astype(bool), labels, confidences.cpu().numpy().tolist()
...
def masks_to_labelmap(masks, class_names, confidences, h, w):
    ...
    for mask, class_name, conf in zip(masks, class_names, confidences):
        ...
        # Grounding DINO may return labels not exactly matching our prompt
...
def process_sequence(seq_path, sam2_predictor, grounding_model, device, args):
    """Process all frames in one sequence folder."""
    ...
    if not os.path.isdir(image_dir):
        print(f"  Skipping {seq_name}: no images/ directory")
        return 0
    ...
    for img_path in tqdm(img_files, desc=f"  {seq_name}", leave=False):
        ...
        try:
            masks, class_names, confidences = segment_frame(
    ...
    if skipped > 0:
        print(f"  {seq_name}: processed {processed}, skipped {skipped} (already done)")
    else:
        print(f"  {seq_name}: processed {processed} frames")
    return processed
...
def main():
    ...
    print(f"\nProcessing {len(seq_folders)} sequences from {args.data_root}")
    print(f"  Stride: {args.stride} | Box threshold: {args.box_threshold} | Text threshold: {args.text_threshold}")
    print(f"  Categories: {CATEGORIES}")
    print(f"  Visualise: {args.visualise} | Force: {args.force}")
    print()
